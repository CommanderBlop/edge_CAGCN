...
def read_cf_list(file_name):
    return np.loadtxt(file_name, dtype=np.float)  # [u_id, i_id]
    # return np.genfromtxt(file_name, dtype=None)
...
def process(train_data, test_data):
    ...
    for u_id, i_id,e_weight in train_data:
        train_user_set[int(u_id)].append(int(i_id))
        train_item_set[int(i_id)].append(int(u_id))
        ratings[(int(u_id), int(i_id))] = e_weight
        ratings[(int(i_id), int(u_id))] = e_weight
    for u_id, i_id,e_weight in test_data:
        test_user_set[int(u_id)].append(int(i_id))

    return np.int32(n_users), np.int32(n_items), train_user_set, test_user_set, train_item_set, ratings
